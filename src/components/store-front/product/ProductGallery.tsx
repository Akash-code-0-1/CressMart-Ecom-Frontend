...
export const ProductGallery: React.FC<GalleryProps> = ({ items }) => {
  ...
  const getUsableImage = (item: MediaItem) => {
    const rowImage = resolveImageSrc(item?.src);
    if (!rowImage) return "/placeholder.png";
    return rowImage.startsWith("http")
      ? rowImage
      : `${backendBaseUrl}/${rowImage.replace(/^\/+/, "")}`;
  };

  const activeCaption = items[activeIndex]?.caption;
    ...
    <div className="productgallery w-full max-w-6xl mx-auto font-poppins">
      <div className="block md:flex gap-4 md:h-[600px]">
        ...
        <div className="w-full md:flex-1 h-[350px] sm:h-[450px] md:h-full rounded-2xl md:rounded-3xl bg-white overflow-hidden relative group border border-gray-100 mb-4 md:mb-0">
          ...
          >
            {items.map((item, index) => {
              const useAbleImage = getUsableImage(item);
              return (
                <SwiperSlide
                  key={index}
                  className="w-full h-full flex items-center justify-center bg-gray-50"
                >
                  <div className="relative w-full h-full">
                    <Image
                      src={useAbleImage}
                      alt={
                        item.alt || item.title || `Product view ${index + 1}`
                      }
                      title={item.title}
                      fill
                      priority={index === 0}
                ...
                return (
                  ...
                  >
                    <div className="relative w-full h-full bg-gray-100">
                      <Image
                        src={useAbleImage}
                        alt={item.alt || item.title || `thumbnail ${index}`}
                        fill
                        className="object-cover"
                        unoptimized
                      />
      ...
      {/* Caption below main preview (SEO) */}
      {activeCaption && (
        <p className="text-sm text-gray-500 mt-2 px-1">{activeCaption}</p>
      )}
