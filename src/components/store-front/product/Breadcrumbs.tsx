...
import React from "react";
import { MdChevronRight } from "react-icons/md";
import Link from "next/link"; // Ensure this is the only Link import!
...
export const Breadcrumbs: React.FC<{ paths: PathItem[], activePath: string }> = ({ paths, activePath }) => {
  return (
    <nav className="flex items-center gap-1 sm:gap-2 mb-6 sm:mb-8 flex-wrap font-poppins pt-6">
      {/* This Link will now work as a navigation element */}
      <Link href="/" className="text-[#727272]">Home</Link>
      <MdChevronRight className="text-[#727272] text-xl" />
      
      {paths.map((p, i) => (
        <React.Fragment key={i}>
          <Link href={`/category/${p.slug}`} className="text-[#727272] hover:text-[#FF7050] text-base font-medium whitespace-nowrap">
            {p.name}
          </Link>
          <MdChevronRight className="text-[#727272] text-xl" />
        </React.Fragment>
