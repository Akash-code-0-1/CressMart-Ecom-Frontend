...
const TestimonialsManager = () => {
  ...
  const mutation = useMutation({
    mutationFn: (data: any) => {
      const payload = { ...data };
      if (!payload.video_url || payload.video_url.trim() === "") payload.video_url = null;
      if (!payload.author_avatar || payload.author_avatar.trim() === "") payload.author_avatar = null;
      if (!payload.thumbnail || payload.thumbnail.trim() === "") payload.thumbnail = null;
  ...
  const handleFileUpload = async (
    ...
    try {
      const uploadRes = await apiFetch("/testimonials/upload", {
        method: "POST",
        body,
      });
      const result = await uploadRes.json();

      // Deep extraction to safely handle nested interceptor payloads
      const url =
      ...
      if (!url) {
        toast.error("Could not parse image URL from response");
        return;
      }
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Manage Facebook reviews and YouTube testimonials
            </p>
          </div>
          <PrimaryButton
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {testimonials.map((item: any) => (
              <div
                key={item.id}
                className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all p-5 flex flex-col group relative"
              >
                <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity z-20">
                  ...
                  <button
                    onClick={() =>
                      confirm("Delete?") && deleteMutation.mutate(item.id)
                    }
                ...
                <div className="flex items-center gap-3 mb-4">
                  <div className="relative w-12 h-12 rounded-full overflow-hidden border border-gray-100 bg-gray-50">
                    <Image
                      src={getFullUrl(item.author_avatar)}
                      alt=""
                      fill
                ...
                <p className="text-xs text-gray-600 leading-relaxed line-clamp-4 flex-1 italic">
                  "{item.content}"
                </p>
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col">
            <div className="p-6  flex justify-between items-center bg-gray-50">
              <h3 className="font-bold text-gray-900">
                {editingId ? "Edit" : "New"} Testimonial
              </h3>
              <button
                onClick={closeModal}
            ...
            >
              ...
              {formData.type === "FACEBOOK" ? (
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1 text-center">
                    <label className="text-[11px] font-bold text-gray-400 uppercase block text-left">
                      Avatar
                    </label>
                    <label className="flex items-center justify-center gap-2 border-dashed border-2 border-gray-200 rounded-xl p-2 cursor-pointer hover:bg-gray-50 transition-all">
                      ...
                      <input
                        type="file"
                        className="hidden"
                        onChange={(e) => handleFileUpload(e, "author_avatar")}
                      />
                    </label>
                  </div>
                </div>
              ...
              <div className="pt-4 flex gap-3">
                <button
                  type="button"
                  onClick={closeModal}
                  className="flex-1 py-3 border border-gray-200 rounded-xl text-sm font-bold text-gray-500 hover:bg-gray-100 cursor-pointer"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 py-3 bg-[#FF7050] hover:bg-black text-white rounded-xl text-sm font-bold shadow-lg shadow-orange-100 transition-all cursor-pointer disabled:opacity-50"
                >
