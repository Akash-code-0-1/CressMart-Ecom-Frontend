...
const ProductAnalytics: React.FC<ProductAnalyticsProps> = ({ bestSellingData, isLoading }) => {
    // 🚀 2. Image Path Logic (Matches your project theme)
    ...
    const getImgUrl = (rawImg: string) => {
        ...
        return rawImg.startsWith("http") ? rawImg : `${baseStorageUrl}/${rawImg.replace(/^\/+/, "")}`;
    };

    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 font-poppins">
            {/* 1. Best Selling Product Section */}
            <div className="lg:col-span-2 bg-white px-6 py-3 rounded-[8px] flex flex-col shadow-sm border border-gray-50">
                <h2 className="text-lg font-bold text-[#23272E] font-lato mb-2">Best Selling Product</h2>
                ...
                <div className="overflow-x-auto flex-grow">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-gray-50">
                                    ...
                                    <tr key={idx} className="text-sm">
                                        <td className="py-4 px-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                />
                                                <span className="font-semibold text-[#0F2D37] truncate max-w-[200px]">{item.name}</span>
                                            </div>
                                        </td>
                                        <td className="py-4 px-4 text-slate-600 font-medium">{item.totalOrder}</td>
                                        <td className="py-4 px-4">
                                            <div className="flex items-center gap-2">
                                                <span className={`w-2 h-2 rounded-full ${item.status === "Stock" ? "bg-green-500" : "bg-red-500"}`}></span>
                                                <span className={item.status === "Stock" ? "text-green-500 font-medium" : "text-red-500 font-medium"}>{item.status}</span>
                                            </div>
                                        </td>
                                        <td className="py-4 px-4 font-bold text-[#0F2D37]">৳{item.price}</td>
                                    </tr>
                            ...
                            {!isLoading && bestSellingData?.length === 0 && (
                                <tr>
                                    <td colSpan={4} className="py-10 text-center text-gray-400 italic">No sales data found for this period.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
            ...
            {/* 2. Top Products Section (Visual Search Sidebar) */}
