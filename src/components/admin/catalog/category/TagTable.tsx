...
//     mutationFn: (ids: string[]) => bulkDeleteTags(ids),
//     onSuccess: (res: any) => {
//       queryClient.invalidateQueries({ queryKey: ["catalog-tags-list"] });
//       setSelectedIds([]);
//       toast.success(res?.message || "Selected tags deleted successfully.");
//     },
//     onError: (err: any) => toast.error(err.message),
//   });
...
//   const handleSelectAll = () => {
//     if (selectedIds.length === tagList.length && tagList.length > 0) {
//       setSelectedIds([]);
//     } else {
//       setSelectedIds(tagList.map((item: Tag) => item.id));
//     }
//   };
...
//         <div className="flex items-center justify-between bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg mb-4 mx-2 md:mx-0 transition-all">
//           <span className="text-sm font-medium">
//             {selectedIds.length} tag(s) selected
//           </span>
//           <button
//             onClick={handleBulkDelete}
//             disabled={bulkDeleteMutation.isPending}
//             className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white text-xs px-3 py-2 rounded-md font-medium cursor-pointer transition-colors disabled:opacity-50"
//           >
      ...
      {activeMenuId && (
        ...
        >
          ...
          <div className="px-2">
            <button
              onClick={() => {
                if (window.confirm("Delete this campaign tag permanently?")) {
