...
//   // ChildCategoryTable.tsx

// const { data: serverPayload, isLoading } = useQuery({
//   queryKey: ["catalog-childcategories-list", page, limit, search, status],
//   queryFn: () => {
//     return fetchAllChildCategories({ // or your child category specific service
//       page,
//       limit,
//       search: search || undefined,
//       status: status || undefined,
//       level: 3, // 👈 🚀 This ensures ONLY Level 3 (Child Categories) appear
//     });
//   },
// });
...
//     onSuccess: () => {
//       queryClient.invalidateQueries({ queryKey: ["catalog-childcategories-list"] });
//       alert("Selected child categories deleted successfully.");
//       setSelectedIds([]);
//     },
//     onError: (err: any) => alert(err?.response?.data?.message || err.message),
//   });
...
//     if (
//       confirm(
//         `Are you sure you want to delete ${selectedIds.length} selected child categories?`
//       )
//     ) {
...
//                   }
//                 }}
//                 className="w-full text-left px-4 py-2 text-xs text-red-600 hover:bg-red-50 flex items-center gap-2 cursor-pointer font-medium disabled:opacity-50"
//               >
...
//   return (
//     <div className="bg-white font-poppins">
//       {/* Selected Action Bar for Bulk Deletion */}
//       {selectedIds.length > 0 && (
//         <div className="flex items-center justify-between px-6 py-3 bg-red-50 border-b border-red-100">
//           <span className="text-xs text-red-700 font-medium">
//             {selectedIds.length} child category item(s) selected
//           </span>
//           <button
//             onClick={handleBulkDelete}
//             disabled={bulkDeleteMutation.isPending}
//             className="flex items-center gap-1.5 px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white rounded text-xs font-medium transition cursor-pointer disabled:opacity-50"
//           >
  ...
  const { data: serverPayload, isLoading } = useQuery({
    ...
    queryFn: () => {
      return fetchAllChildCategories({
        ...
        search: search || undefined,
        status: status || undefined,
        level: 3, // Ensures ONLY Level 3 appear
      });
    },
  });
      ...
      {activeMenuId && (
        ...
        >
          ...
          <div className="px-2">
            <button
              onClick={() => {
                if (window.confirm("Delete this child category permanently?")) {
