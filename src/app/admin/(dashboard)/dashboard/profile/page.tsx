...
import { FaSpinner, FaCheckCircle, FaExclamationCircle } from "react-icons/fa";
import { useAdminProfileData, useAdminUpdateProfileMutation, useAdminUpdateAvatarMutation } from "@/hooks/useProfile";
import { User, Phone, Mail, Shield, Camera, Loader2, LogOut, LogIn } from "lucide-react"; 
import { useAuthStore } from "@/store/useAuthStore";
import { deleteAdminSessionToken } from "@/app/actions/auth"; // CHANGED: Use the Admin specific delete action
...
const AdminProfilePage = () => {
  ...
  // Auth Store - Use admin specific clear if available, otherwise clearAuth
  const clearAdminAuth = useAuthStore((state) => state.clearAdminAuth || state.clearAuth);
  const adminUser = useAuthStore((state) => state.adminUser || state.user);
  ...
  const handleLogout = async () => {
    try {
      ...
      // 3. Delete the specific Admin Cookie
      await deleteAdminSessionToken(); 
      
      // 4. Refresh and Redirect to Admin Signin
      router.refresh();
  ...
  // Double check signed in status
  const isSignedIn = !!profile || (isHydrated && !!adminUser);
          ...
          <div className="flex justify-between items-center mt-4 pt-4 border-t border-gray-100">
            {isSignedIn ? (
              ...
              <Link
                ...
                className="flex items-center gap-2 px-4 py-2 text-[#FF7050] hover:bg-orange-50 rounded-[8px] transition-all font-semibold text-sm cursor-pointer"
              >
                <LogIn size={18} />
                Sign In
              </Link>
