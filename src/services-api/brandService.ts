...
// 🚀 4. BULK DELETE BRANDS
export const bulkDeleteBrands = async (ids: string[]) => {
  const token = await getAdminTokenAction();
  const res = await apiFetch("/brand/bulk-delete", {
    method: "DELETE",
    headers: {
      Authorization: `Bearer ${token || ""}`,
      "Content-Type": "application/json",
  ...
  if (!res.ok) {
    const errorJson = await res.json();
    throw new Error(
      errorJson?.message || "Failed to bulk delete selected brand records.",
    );
  }
  return res.json();
};

// 🚀 5. UPLOAD BRAND GRAPHIC LOGO TO STORAGE
export const uploadBrandImage = async (file: File) => {
  const token = await getAdminTokenAction();
  const formData = new FormData();
  formData.append("image", file);
...
// 🚀 7. UPDATE EXISTING BRAND RECORD
export const updateBrand = async (
  id: string,
  payload: {
    name: string;
